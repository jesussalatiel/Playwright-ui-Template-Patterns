...
export async function clickAndNavigate(
  ...
  try {
    ...
    await Promise.all([
      click(input, options),
      getPage().waitForEvent("framenavigated", { timeout: timeout + 100 }),
    ]);
    await getPage().waitForLoadState(
      options?.loadState || getDefaultLoadState(),
      {
        timeout,
      },
    );
    ...
    if (
      ...
      error.message.includes("framenavigated")
    ) {
      throw new Error(
        `After the click action, the page did not navigate to a new page\n ${error.message}`,
      );
    } else {
      throw error;
    }
 ...
 */
export async function pressPageKeyboard(
  key: string,
  options?: PressSequentiallyOptions,
): Promise<void> {
...
export async function acceptAlert(
  ...
  const dialogPromise = getPage()
    ...
    .catch(() => {
      throw new Error(
        `No dialog appeared after waiting for ${timeoutInMs} ms.`,
      );
    });
