...
export async function getLocatorCount(
  ...
  try {
    ...
    logger.error(
      `getLocatorCount- ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return 0;
}
...
export async function isElementAttached(
  ...
  try {
    ...
    logger.error(
      `isElementAttached- ${error instanceof Error ? error.message : String(error)}`,
    );
    return false;
  }
}
...
export async function isElementVisible(
  ...
  try {
    ...
    logger.error(
      `isElementVisible- ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return false;
}
...
export async function isElementHidden(
  ...
  try {
    ...
    logger.error(
      `isElementHidden- ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return false;
}
...
export async function isElementChecked(
  ...
  try {
    ...
    logger.error(
      `isElementChecked- ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return false;
}
...
export async function waitForElementToBeVisible(
  ...
  await locator.waitFor({
    state: "visible",
    timeout: options?.timeout || SMALL_TIMEOUT,
  });
}
