...
export function getLocator(
  ...
  const locator =
    typeof input === "string" ? getPage().locator(input, options) : input;
 ...
 */
export function getVisibleLocator(
  input: string | Locator,
  options?: LocatorOptions,
): Locator {
...
export async function getAllLocators(
  ...
  return typeof input === "string"
    ? getPage().locator(input, options).all()
    : input.all();
}
...
export function getFrame(
  ...
  if (!frame) {
    throw new Error(
      `Frame not found with selector: ${JSON.stringify(frameSelector)}`,
    );
  }
  return frame;
}
