...
import { getDefaultLoadState, SMALL_TIMEOUT } from "../constants";
import {
  GotoOptions,
  NavigationOptions,
  SwitchPageOptions,
...
export async function switchPage(
  ...
  while (
    getAllPages().length < winNum &&
    Date.now() - startTime < timeoutInMs
  ) {
  ...
  expect(
    getAllPages().length,
    `Page number ${winNum} not found after ${timeoutInMs} seconds`,
...
export async function clearCookies(options?: any): Promise<void> {
  await getContext().clearCookies(options);
}
