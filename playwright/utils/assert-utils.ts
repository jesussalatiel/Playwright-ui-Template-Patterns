...
import { getAllPages, getPage, waitForPageLoadState } from "./page-utils";
import { getAlertText } from "./action-utils";

/**
 * Enum for specifying the type of comparison.
 * @readonly
 * @enum {number}
 */
 ...
 * @param {string | Locator} input - CSS selector or Locator of the element.
 * @param {string | RegExp | Array<string | RegExp>} text - Class or class pattern to verify.
 * @param {ExpectOptions & ExpectTextOptions} [options] - Additional options for verification.
...
export async function expectElementToHaveClass(
  ...
  const { locator, assert } = getLocatorAndAssert(input, options);
  await assert(locator, options).toHaveClass(text, options);
}
 ...
 * @param {string} value - Expected value of the CSS property.
 * @param {ExpectOptions & ExpectTextOptions} [options] - Additional options for verification.
 * @returns {Promise<void>}
 */
export async function expectElementToHaveCss(
  ...
  const { locator, assert } = getLocatorAndAssert(input, options);
  await assert(locator, options).toHaveCSS(name, value);
}
 ...
 * @returns {void}
 */
export function expectPageSizeToBeEqualTo(
...
/**
 * Compares the current page or content at a locator to a stored snapshot.
 *
 * @param {ComparisonBy} comparison - The type of comparison to perform.
 * @param {string} [locator] - The locator for the element whose content is to be compared. Required if comparison is ComparisonBy.Content.
 * @param {object} [options={}] - Options to pass to the snapshot comparison.
 * @returns {Promise<void>} A promise that resolves when the comparison is complete.
 * @throws {Error} Throws an error if locator is not provided for ComparisonBy.Content.
 */
export async function expectedToMatchSnapshot(
  comparison: ComparisonBy,
  locator?: string,
  options?: any,
): Promise<void> {
  ...
  switch (comparison) {
    case ComparisonBy.Content:
      snapshot = await page.textContent(locator);
      break;
    case ComparisonBy.Page:
      snapshot = await page.screenshot();
      break;
    default:
  }

  expect(snapshot).toMatchSnapshot(options);
}
